...
while true; do
  ...
  echo -e "   ${CYAN}- Ver estado de red, configurar proxy DGES, distribuir script por red, configurar interfaces, probar conectividad${RESET}"
  echo -e "${GREEN}2) 👥 Usuarios y cuentas${RESET}"
  echo -e "   ${CYAN}- Crear, eliminar y modificar usuarios y grupos${RESET}"
  echo -e "${GREEN}3) 💻 Información del sistema${RESET}"
  echo -e "   ${CYAN}- Ver información de hardware, software y recursos del sistema${RESET}"
  echo -e "${GREEN}4) 🧹 Mantenimiento${RESET}"
  echo -e "   ${CYAN}- Limpiar archivos temporales, actualizar sistema, liberar espacio${RESET}"
  echo -e "${GREEN}5) 🧩 Instalaciones frecuentes${RESET}"
  echo -e "   ${CYAN}- Instalar programas y utilidades comunes${RESET}"
